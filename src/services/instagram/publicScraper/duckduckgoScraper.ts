/**
 * DuckDuckGo Search-based Instagram Scraper
 *
 * Uses DuckDuckGo to find Instagram reels/posts without authentication.
 * DuckDuckGo has more lenient bot detection compared to Google.
 *
 * @module services/instagram/publicScraper/duckduckgoScraper
 */

import { BuzzReel } from '../../../types/index.js';

/**
 * Configuration for DuckDuckGoScraper
 */
...
const DEFAULT_CONFIG: Required<DuckDuckGoScraperConfig> = {
  browser: 'chromium',
  headless: true,
  timeout: 30000,
...
/**
 * DuckDuckGoScraper - Find Instagram content via DuckDuckGo Search
 *
 * This scraper works by:
 * 1. Performing DuckDuckGo searches with site:instagram.com/reel
 * 2. Extracting Instagram URLs from search results
 * 3. Fetching reel details using oEmbed API
 */
export class DuckDuckGoScraper {
  private config: Required<DuckDuckGoScraperConfig>;
  private playwright: any = null;
  private browser: any = null;

  constructor(config: DuckDuckGoScraperConfig = {}) {
  ...
  private async initBrowser(): Promise<void> {
    ...
    try {
      ...
      console.log(`[DuckDuckGoScraper] Browser initialized (${this.config.browser})`);
    } catch (error) {
      console.error('[DuckDuckGoScraper] Failed to initialize browser:', error);
  ...
  /**
   * Search DuckDuckGo for Instagram reels by hashtag
   *
  ...
  async searchByHashtag(hashtag: string, limit: number = 20): Promise<string[]> {
    ...
    console.log(`[DuckDuckGoScraper] Searching DuckDuckGo for Instagram reels with #${tag}`);
    ...
    try {
      ...
      const duckduckgoUrl = `https://duckduckgo.com/?q=${encodeURIComponent(searchQuery)}&t=h_&ia=web`;
      ...
      // Click "More results" if available to get more results
      try {
        const moreButton = await page.$('a.result--more__btn, button:has-text("More results")');
        if (moreButton) {
          await moreButton.click();
          await page.waitForTimeout(2000);
        }
      } catch {
        // More button may not be present
      }
      ...
      const shortcodes = await page.evaluate(() => {
        ...
        const links = document.querySelectorAll('a[href*="instagram.com/reel/"], a[data-testid="result-title-a"][href*="instagram.com"]');
        ...
        // Also check result snippets for Instagram URLs
        const snippets = document.querySelectorAll('.result__snippet, .result__body');
        snippets.forEach(snippet => {
          const text = snippet.textContent || '';
          const matches = text.matchAll(/instagram\.com\/reel\/([A-Za-z0-9_-]+)/g);
      ...
      console.log(`[DuckDuckGoScraper] Found ${shortcodes.length} reel shortcodes from DuckDuckGo`);
      ...
      console.error('[DuckDuckGoScraper] Error searching DuckDuckGo:', error);
      return [];
    } finally {
      if (page) {
  ...
  /**
   * Search DuckDuckGo for Instagram reels by keyword
   *
  ...
  async searchByKeyword(keyword: string, limit: number = 20): Promise<string[]> {
    console.log(`[DuckDuckGoScraper] Searching DuckDuckGo for Instagram reels with keyword: ${keyword}`);
    ...
    try {
      ...
      console.log(`[DuckDuckGoScraper] Found ${shortcodes.length} shortcodes from DuckDuckGo`);
  ...
  /**
   * Get BuzzReels by hashtag using DuckDuckGo Search
   *
  ...
  async getReelsByHashtag(hashtag: string, limit: number = 20): Promise<BuzzReel[]> {
    ...
    for (const shortcode of shortcodes) {
      try {
        ...
        console.error(`[DuckDuckGoScraper] Error fetching reel ${shortcode}:`, error);
      }
  ...
  /**
   * Get BuzzReels by keyword using DuckDuckGo Search
   *
   * @param keyword - The keyword to search for
   * @param limit - Maximum number of results
   * @returns Array of BuzzReel objects
  ...
  private async getReelDetails(shortcode: string): Promise<BuzzReel | null> {
    try {
      ...
      console.error(`[DuckDuckGoScraper] Error getting reel details for ${shortcode}:`, error);
      return null;
    }
  }
  ...
  async close(): Promise<void> {
    if (this.browser) {
      ...
      this.browser = null;
      console.log('[DuckDuckGoScraper] Browser closed');
    }
  }
...
export const duckduckgoScraper = new DuckDuckGoScraper();
